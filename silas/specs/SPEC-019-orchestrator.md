...
### 4.0 Dispatch Scout for Site Crawl (MANDATORY FIRST STEP)

**Before doing ANY audit work, Silas MUST dispatch Scout to crawl the target website.**

Scout performs a full technical site crawl — fetching every page, parsing HTML, extracting meta tags, OG tags, images, schema, internal links, and producing a structured crawl report. This is the equivalent of what Screaming Frog or Sitebulb would produce.
...
EXPECTED OUTPUT: Structured crawl report (see Scout AGENTS.md § Site Crawl Mission)
BLOCKING: Yes — do NOT proceed to Phase 2 (Audit) until Scout's crawl report is received
```

**What Scout's crawl report provides:**
- Exact title tags, meta descriptions, H1s for every page (no more UNKNOWN)
- OG tags and Twitter Card values (present or missing, with exact content)
- Image inventory: total count, alt text coverage percentage
- Schema markup: every JSON-LD block parsed and typed
- robots.txt contents and sitemap data
- Broken links, redirect chains, orphan pages
- Page-by-page inventory classified by type (service, location, blog, other)
- Site-wide aggregate metrics

**If Scout's crawl fails or is incomplete:**
- Note which sections have incomplete data in the audit
- Mark affected scores with "LIMITED DATA" rather than "UNKNOWN"
- Flag for operator that a manual crawl tool (Screaming Frog) may be needed

### 4.1 Website Audit Data (from Scout's Crawl Report)

Silas consumes Scout's crawl report and extracts the following for scoring. All of this data should come directly from Scout's output — do NOT guess or mark as UNKNOWN if Scout provided the data.

```
From Scout's crawl report, extract:
1. Homepage analysis:
   ☐ Title tag, meta description, H1 (exact text from Scout)
   ☐ OG tags — og:title, og:description, og:image (from Scout)
   ☐ Twitter Card tags (from Scout)
   ☐ Schema markup — all JSON-LD blocks (from Scout)
   ☐ Canonical tag (from Scout)
   ☐ Core Web Vitals (from Scout via PageSpeed API, or separate API call)
   ☐ Mobile friendliness (viewport tag presence from Scout)
   ☐ SSL status (from Scout)
   ☐ CMS detection (from Scout or auto-detect)

2. Sitemap & robots.txt:
   ☐ robots.txt contents (from Scout — exact text)
   ☐ XML sitemap status (from Scout — URL count, last modified)

3. Full site inventory (from Scout):
   ☐ All internal URLs with page type classification
   ☐ Per-page: title, meta desc, H1, H2s, word count, schema
   ☐ Internal link structure + orphan page detection
   ☐ Image alt text audit (site-wide coverage percentage)
   ☐ Broken links — 4xx, 5xx (from Scout)
   ☐ Redirect chains — 301/302 (from Scout)
   ☐ Duplicate titles and meta descriptions (from Scout)

4. Content inventory (from Scout):
   ☐ Service pages (list all with word counts)
   ☐ Location pages (list all with word counts)
   ☐ Blog posts (list all with dates)
   ☐ About/contact pages
   ☐ Missing pages (services without pages — Silas infers from business type)
```
