         ...
         <div className="mt-auto pt-6 border-t border-border flex flex-col gap-2">
            <div className="flex items-center justify-between text-xs text-muted-foreground">
             ...
             <div className="flex items-center justify-between text-xs text-muted-foreground">
                ...
                {task.totalCost != null && (
                    <div className="flex items-center gap-1.5 text-[var(--accent-orange)] font-medium">
                        <span>${task.totalCost.toFixed(4)}</span>
                        {task.inputTokens != null && task.outputTokens != null && (
                            <span className="text-muted-foreground font-normal">
                                ({formatTokens((task.inputTokens || 0) + (task.outputTokens || 0) + (task.cacheReadTokens || 0) + (task.cacheWriteTokens || 0))} tok)
                            </span>
                        )}
                    </div>
                )}
