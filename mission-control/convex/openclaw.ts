...
export const receiveAgentEvent = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.action === "start") {
			if (!task) {
				// No pre-existing task — create one from scratch
				...
				// Task pre-created by dispatcher — update it with runtime info
				...
				// Update title/description if we have a prompt and title is generic
				if (args.prompt && task.title.startsWith("Agent task")) {
					patch.title = summarizePrompt(args.prompt);
					patch.description = args.prompt;
				}
				// Assign the agent if not already assigned
				if (agent && !task.assigneeIds.includes(agent._id)) {
					patch.assigneeIds = [...task.assigneeIds, agent._id];
				}
				...
				if (agent) {
					...
					await ctx.db.insert("messages", {
						taskId: task._id,
						fromAgentId: agent._id,
						content: `🚀 **Started**\n\n${sourcePrefix}${args.prompt || task.description}`,
						attachments: [],
						tenantId,
					});
					await ctx.db.insert("activities", {
				...
				// Task exists in another state (review, done) — just update start time
