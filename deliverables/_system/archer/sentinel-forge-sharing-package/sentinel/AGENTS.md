...
### Check 9: Cost Anomaly Detection [NEW - Added by Forge 2026-02-15]

**Purpose:** Detect when a single agent consumes disproportionate resources, signaling potential runaway loops, inefficient prompting, or unexpected usage patterns.

**Method:**

1. **Get fleet cost data** using dispatcher cost command:
   ```bash
   node "C:/Users/spart/.openclaw/cron/dispatcher.js" --costs 1
   ```

2. **Parse output** to extract:
   - Total fleet cost (last 24h)
   - Per-agent cost breakdown
   - Identify any agent exceeding 50% of total

3. **Calculate thresholds:**
   - **WARN threshold:** Any agent > 40% of daily total
   - **FAIL threshold:** Any agent > 60% of daily total OR total daily cost > $50

**What to flag:**
- ❌ Main spent $30 out of $40 total (75%) → FAIL
- ⚠️ Silas spent $8 out of $20 total (40%) → WARN
- ✅ Costs evenly distributed, no agent > 40% → PASS

**Status logic:**
- All agents < 40% of total AND total < $50/day = PASS
- Any agent 40-60% of total OR total $30-50/day = WARN
- Any agent > 60% of total OR total > $50/day = FAIL

**Report format for WARN/FAIL:**
```
COST ALERT: {agent} spent ${X} ({Y}% of daily total ${Z})
Breakdown: {top 3 agents with costs}
Action: Review {agent}'s recent sessions for inefficiency
```
